...
export async function ChecklistWithTitle({ data, sectionCount }) {
  const { fields } = data
  ...
  if (_.get(fields, 'contentCollection')) {
    ...
    let temp = _.filter(collections[0].items, data => data.fields.active)
    
    if (temp.length % 2 !== 0) {
      lastCollection = temp[temp.length-1];
      temp = temp.slice(0, -1);
    }
      ...
      <Container width={width} margin="section" className="relative">
...

        {
          subCollection.length > 0 && (
            <Motion>
              <div className={`mt-6 grid  gap-x-8 gap-y-10 text-sm md:text-base leading-7 text-gray-600 ${_.get(fields, 'gridClass')}`}>
                {
              ...
              {
                lastCollection && (
                  <div className="mt-10 grid gap-x-8 gap-y-10 text-sm md:text-base leading-7 text-gray-600">
                    <CheckList item={lastCollection}/>
                  </div>
  ...
  return (
    <MotionVariant variants={fadeInFromBottom} className="relative pl-9">
      <div>
        ...
        <div>
          <h3 className="text-lg font-bold text-gray-900">{_.get(item, 'fields.title')}</h3>
          <p className="font-normal text-gray-900">{_.get(item, 'fields.body')}</p>
        </div>
